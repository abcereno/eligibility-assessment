...
export default function PdfTemplate({ person, date, qualificationName, progress, pageData, onRender }) {
  const { isContinuation, units } = pageData;
  const { evidencePercent, refereePercent, gapPercent } = progress;

  // This effect will call the onRender callback after the component has rendered to the DOM.
  useEffect(() => {
    if (onRender) {
      onRender();
    }
